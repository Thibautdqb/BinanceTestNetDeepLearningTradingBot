...
def main():
    while True:
        a = st.title("Deep learning Tradingbot")
        a
        b= st.header('Python program that executes ETH buy and sell orders on the Binance test platform using signals generated by a Deep Learning program.')
        b
        c = st.write('''Define the hyperparameter windows you wish to use to launch the program and click on the "Start the Magic" button''')
        c
        api_key, api_secret = load_binance_api_keys()
        binance = initialize_binance(api_key, api_secret)
        d = st.title("Formulaire d'adresse e-mail")
        d
        ...
        e= st.write('''Models Hyperparameters Search''')
        e
